...
public class Graph implements Cloneable {
	...
	public List<Pair<Integer, Byte>> getItemsRatedByUser(int user) {
		return adjacencyList.get(user);
	}

	public List<Integer> getUsersRatedItem(int item) {
		return adjacencyList.entrySet().stream().filter(x -> x.getValue().stream().anyMatch(y -> y.getKey() == item))
